...
function readRegistryText(filePath, fsImpl = fs) {
  let stat;
  try { stat = fsImpl.lstatSync(filePath); }
  catch (error) {
    if (error?.code === 'ENOENT') return null;
    throw registryError('EMODELREGISTRY', `Unable to inspect model registry: ${filePath}.`, { cause: error, configPath: filePath });
  }
  ...
  catch (error) { throw registryError('EMODELREGISTRY', `Unable to read model registry: ${filePath}.`, { cause: error, configPath: filePath }); }
}

function parseModelRegistryDocument(text, sourcePath = '<memory>') {
  let document;
  try { document = JSON.parse(String(text || '')); }
  catch (error) { throw registryError('EMODELREGISTRY', `Model registry is invalid JSON: ${sourcePath}.`, { cause: error, configPath: sourcePath }); }
  if (!document || typeof document !== 'object' || Array.isArray(document) || Number(document.schemaVersion) !== MODEL_REGISTRY_SCHEMA_VERSION) {
    throw registryError('EMODELREGISTRY', `Model registry must use schemaVersion ${MODEL_REGISTRY_SCHEMA_VERSION}.`, { configPath: sourcePath });
  }
  ...
  try { return validateModelRegistry({ revision: document.revision, models: document.models }); }
  catch (error) { throw registryError('EMODELREGISTRY', `Model registry validation failed: ${error.message}`, { cause: error, configPath: sourcePath }); }
}

function resolveModelRegistry(value = {}, { env = process.env, homeDir = os.homedir(), fsImpl = fs } = {}) {
  if (value?.registry !== undefined) {
    try {
      return Object.freeze({ registry: validateModelRegistry(value.registry), source: 'explicit', configPath: '', inherited: false });
    } catch (error) {
      throw registryError('EMODELREGISTRY', `Explicit model registry validation failed: ${error.message}`, { cause: error });
    }
  }
  const filePath = value?.registryFile
    ? (path.isAbsolute(String(value.registryFile)) ? path.normalize(String(value.registryFile)) : path.resolve(homeDir, String(value.registryFile)))
    : modelRegistryPath(env, homeDir);
...
function inspectModelRegistry(value = {}, options = {}) {
  ...
  if (!resolved.registry) return Object.freeze({ source: resolved.source, configPath: resolved.configPath, present: false, revision: '', models: 0, approved: 0 });
  const approved = resolved.registry.models.filter(model => model.status === 'approved' && model.health !== 'unhealthy').length;
  return Object.freeze({
    source: resolved.source,
    configPath: resolved.configPath,
    present: true,
    revision: resolved.registry.revision,
    models: resolved.registry.models.length,
    approved
  });
}
