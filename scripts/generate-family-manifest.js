...
for(const name of CONSUMERS){
  const cwd=path.join(familyDir,name),sha=git(['rev-parse','HEAD'],cwd),pin=git(['ls-files','--stage','src/codex-safe-core'],cwd).split(/\s+/)[1]||'',recorded=snapshot.consumers?.[name],pinnedRoot=path.join(cwd,'src','codex-safe-core');
  ...
  if(productContract.productContractVersion!==contract.productContractVersion)throw new Error(`${name} Product Contract version drift.`);
  ...
  if(productContract.safeCoreCommit!==pin||productContract.safeCoreVersion!==recorded.corePin.version)throw new Error(`${name} Product Contract pinned Core drift.`);
  if(productContract.safeCoreRuntimeDigest!==pinnedDigests.runtimeDigest||productContract.safeCoreGovernanceDigest!==pinnedDigests.governanceDigest)throw new Error(`${name} Product Contract digest drift.`);
  consumers[name]={
