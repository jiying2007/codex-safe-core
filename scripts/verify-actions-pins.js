...
function walk(p){if(!fs.existsSync(p))return[];const s=fs.lstatSync(p);if(s.isSymbolicLink())throw new Error(`workflow scan must not traverse symlink: ${p}`);if(s.isFile())return[p];return fs.readdirSync(p,{withFileTypes:true}).flatMap(e=>walk(path.join(p,e.name)));}
let checked=0;
