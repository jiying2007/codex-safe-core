...
async function inspectCore({token=process.env.GITHUB_TOKEN}={}){
  const commit=await githubJson(`/repos/${OWNER}/${CORE_REPO}/commits/main`,{token}),sourceSha=commit.sha;if(!isSha(sourceSha))throw new Error(`${CORE_REPO} main did not resolve to an exact SHA.`);
  const pkg=await rawJson(CORE_REPO,sourceSha,'package.json'),version=String(pkg.version||''),release=await inspectRelease(CORE_REPO,version,sourceSha,{token}),contractAligned=version===contract.coreVersion,digests=await inspectCoreDigests(release,{token}),releaseReady=release.ready&&contractAligned&&digests.ready;
  return{sha:sourceSha,version,releaseReady,release,digests,reason:!contractAligned?'version-contract-drift':!release.ready?release.reason:digests.reason};
}
async function inspectConsumer(repo,core,{token=process.env.GITHUB_TOKEN}={}){
  const commit=await githubJson(`/repos/${OWNER}/${repo}/commits/main`,{token}),sourceSha=commit.sha;if(!isSha(sourceSha))throw new Error(`${repo} main did not resolve to an exact SHA.`);
  const[pkg,productContract,corePin]=await Promise.all([rawJson(repo,sourceSha,'package.json'),rawJson(repo,sourceSha,'product-contract.json'),githubJson(`/repos/${OWNER}/${repo}/contents/src/codex-safe-core?ref=${encodeURIComponent(sourceSha)}`,{token})]);
  ...
  const release=await inspectRelease(repo,version,sourceSha,{token});
  const ciReceipt=await inspectConsumerCiReceipt(repo,version,sourceSha,release,productContract,pinnedCoreDigests,{token});
  const requiredAssets=registry.consumers[repo]?.release?.requiredAssets||[],releaseAssetsReady=requiredAssets.every(name=>release.assets.some(asset=>asset.name===name)),distribution=await inspectDistribution(repo,version,sourceSha,release,{token}),releaseReady=aligned&&release.ready&&releaseAssetsReady&&ciReceipt.ready,ready=releaseReady&&(!registry.consumers[repo]?.distribution?.required||distribution.ready);
