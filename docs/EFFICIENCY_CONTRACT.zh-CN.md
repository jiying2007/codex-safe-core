...
Codex Safe Core v4.14.1 为整个产品族统一提供通用的模型成本与执行效率原语。
...
各产品继续负责自己的预算值、质量阈值、确定性 Evidence 持久化与 UI。预算导致的证据遗漏必须显式暴露，禁止静默宣称覆盖完整。持久化 Model Judgment 不属于效率原语：Judgment Lifecycle v1 禁止把历史模型 Finding 当成 fresh 结果或重新参与 Verdict。Codex Change Safe 默认模型调用为 0，因此这些模型效率原语不会给 Change 阶段增加模型请求。
