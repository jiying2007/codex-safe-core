...
function normalizeRegistryModel(raw, index = 0) {
  ...
  return freeze({ id, provider, model, class: modelClass, roles, status, health, priority, qualificationId: clean(raw.qualificationId || '', `registry model ${id} qualificationId`, 256, { allowEmpty: true }), revision: clean(raw.revision || '', `registry model ${id} revision`, 256, { allowEmpty: true }), revisionPinStrength, capabilities: normalizeCapabilities(raw.capabilities || {}) });
}
function validateModelRegistry(registry = {}) {
  ...
  const ids = new Set(), providerModels = new Set();
...
function assessModelCompatibility({ mode = 'balanced', role = 'reviewer', modelClass = 'balanced', policy } = {}) {
  ...
  return freeze({ mode: resolvedMode, role: resolvedRole, modelClass: resolvedClass, requiredClass, compatibilityPolicy, compatible, degraded: !compatible, allowed: compatible || compatibilityPolicy !== 'strict' });
}
...
function modelLookup(registry) { const byIdentity = new Map(); for (const item of registry.models) byIdentity.set(`${item.provider}\n${item.model}`, item); return byIdentity; }
...
function chooseAuto(registry, role, mode, compatibilityPolicy, provider, allowedProviders) {
  return registry.models.filter(model => baseEligibility(model, role, { approvedOnly: true })).filter(model => !provider || model.provider === provider).filter(model => !allowedProviders.length || allowedProviders.includes(model.provider)).map(model => ({ model, compatibility: assessModelCompatibility({ mode, role, modelClass: model.class, policy: compatibilityPolicy }) })).filter(item => item.compatibility.allowed).sort((a, b) => {
    if (a.compatibility.degraded !== b.compatibility.degraded) return a.compatibility.degraded ? 1 : -1;
    const target = CLASS_RANK[requiredClassFor(mode, role)], distanceA = Math.abs(CLASS_RANK[a.model.class] - target), distanceB = Math.abs(CLASS_RANK[b.model.class] - target);
...
function resolveModelSelection(request = {}) {
  ...
  const allowedProviders = Array.isArray(request.allowedProviders) ? [...new Set(request.allowedProviders.map((value, index) => clean(value, `allowedProviders[${index}]`, 128)))] : [];
  const crossProvider = request.crossProvider === true;
  const normalizedCandidates = Array.isArray(request.candidates) ? request.candidates.map((item, index) => normalizeCandidate(item, `candidate ${index}`)) : [];
  const fixed = strategy === 'fixed' ? { provider: provider || clean(request.fixed?.provider || '', 'fixed provider', 128), model: clean(request.model || request.fixed?.model || '', 'fixed model', 256) } : null;
  const routingPolicyDigest = digest({ mode, role, strategy, compatibilityPolicy, provider, allowedProviders, crossProvider, candidates: normalizedCandidates, fixed, requireApprovedFixed: request.requireApprovedFixed === true });
  const byIdentity = modelLookup(registry);
  let selected = null, fallbackUsed = false;
  if (strategy === 'fixed') {
    ...
    for (let index = 0; index < normalizedCandidates.length; index++) {
      ...
      selected = { model, compatibility }; fallbackUsed = index > 0; break;
    }
  } else selected = chooseAuto(registry, role, mode, compatibilityPolicy, provider, allowedProviders);
  if (!selected) {
    const error = new Error(`No eligible model is available for ${role}/${mode} using ${strategy} selection.`);
    error.code = 'MODEL_UNAVAILABLE'; error.role = role; error.mode = mode; error.strategy = strategy; throw error;
  }
  if (!crossProvider && provider && selected.model.provider !== provider) { const error = new Error('Cross-provider model fallback is disabled.'); error.code = 'MODEL_CROSS_PROVIDER_FORBIDDEN'; throw error; }
  return freeze({ contractVersion: MODEL_ROUTING_CONTRACT_VERSION, registryRevision: registry.revision, registryDigest: registry.digest, routingPolicyDigest, mode, role, strategy, compatibilityPolicy, requestedProvider: provider, resolvedProvider: selected.model.provider, resolvedModel: selected.model.model, resolvedModelRevision: selected.model.revision, revisionPinStrength: selected.model.revisionPinStrength, modelClass: selected.model.class, qualificationId: selected.model.qualificationId, fallbackUsed, crossProviderFallback: Boolean(provider && selected.model.provider !== provider), compatibility: selected.compatibility.compatible ? 'compatible' : 'degraded', degraded: selected.compatibility.degraded, capabilities: selected.model.capabilities });
}
function buildModelEvidence(selection, options = {}) {
  ...
  return freeze({ contractVersion: MODEL_ROUTING_CONTRACT_VERSION, mode: selection.mode, role: selection.role, selectionStrategy: selection.strategy, requestedProvider: selection.requestedProvider || '', resolvedProvider: selection.resolvedProvider, resolvedModel: selection.resolvedModel, resolvedModelRevision: selection.resolvedModelRevision || '', modelClass: selection.modelClass, routingPolicyRevision: clean(options.routingPolicyRevision || 'unversioned', 'routingPolicyRevision', 256), routingPolicyDigest: clean(options.routingPolicyDigest || selection.routingPolicyDigest || '', 'routingPolicyDigest', 64, { allowEmpty: true }), registryRevision: selection.registryRevision, registryDigest: clean(options.registryDigest || selection.registryDigest || '', 'registryDigest', 64, { allowEmpty: true }), qualificationId: selection.qualificationId || '', lineagePinned: options.lineagePinned === true, revisionPinStrength: selection.revisionPinStrength || 'unknown', fallbackUsed: selection.fallbackUsed === true, crossProviderFallback: selection.crossProviderFallback === true, compatibility: selection.compatibility, degraded: selection.degraded === true, usage: { inputTokens: number(usage.inputTokens ?? usage.input_tokens), cachedInputTokens: number(usage.cachedInputTokens ?? usage.cached_input_tokens), cacheWriteInputTokens: number(usage.cacheWriteInputTokens ?? usage.cache_write_input_tokens), outputTokens: number(usage.outputTokens ?? usage.output_tokens), reasoningOutputTokens: number(usage.reasoningOutputTokens ?? usage.reasoning_output_tokens) } });
}

module.exports = { MODEL_ROUTING_CONTRACT_VERSION, REVIEW_MODES, REVIEW_MODE_PLANS, MODEL_ROLES, MODEL_CLASSES, MODEL_SELECTION_STRATEGIES, MODEL_COMPATIBILITY_POLICIES, MODEL_STATUSES, MODEL_HEALTH, REVISION_PIN_STRENGTH, resolveReviewModePlan, validateModelRegistry, requiredClassFor, assessModelCompatibility, resolveModelSelection, buildModelEvidence };
