...
const COMMIT_RECEIPT_SCHEMA_VERSION = 1;
...
function validOid(value) { return value === '<unborn>' || /^[0-9a-f]{40,64}$/i.test(value); }
function validHash(value) { return /^[0-9a-f]{64}$/i.test(value); }
function validateReviewReceipt(value) {
  ...
  if (!validOid(value.headOid) || !validHash(value.indexFingerprint) || !validHash(value.diffFingerprint)) return null;
  if (value.policyFingerprint !== '<none>' && !validHash(value.policyFingerprint)) return null;
...
function validateCommitReceipt(value) {
  ...
  for (const key of ['headOid','indexFingerprint','diffFingerprint','messageFingerprint','createdAt']) {
    if (typeof value[key] !== 'string' || !value[key]) return null;
  }
  ...
  if (!Number.isFinite(Date.parse(value.createdAt))) return null;
  if (value.reviewReceiptFingerprint !== undefined && !validHash(value.reviewReceiptFingerprint)) return null;
  return Object.freeze({ ...value });
}
...
module.exports = {
  ...
  buildSafeCodexArgs, missingHelpFlags, isCliCompatibilityError,
  fingerprint, fingerprintPolicy: fingerprint, validateReviewReceipt, validateCommitReceipt
};
